"""The generated capability-and-claim evidence report.

One job: take each claim this repository makes about itself and say **PASS** or
**FAIL**, with the artifact the verdict was read from.

Two kinds of claim are tracked, and they are never mixed:

``implementation``
    A capability exists in committed code, is exercised by the test suite, and
    leaves a generated artifact behind. "The classical baseline is implemented"
    is settled by reading the module and the artifact it wrote; no experiment
    has to have run.

``empirical``
    A number that only a measurement can settle -- how many human labels exist,
    what the held-out NDCG@3 was, whether an improvement cleared a threshold. An
    implemented capability is never evidence for one of these.

Every value here is read out of a generated artifact. Nothing is typed, nothing
...
CLAIM_EVIDENCE_SCHEMA_VERSION = "claim-evidence-v1.0.0"

#: A claim settled by reading committed code, its tests and the artifact it
#: produces. It says the capability exists, never that it measured anything.
IMPLEMENTATION = "implementation"
#: A claim only a measurement can settle. An implemented capability is never
#: evidence for one of these.
EMPIRICAL = "empirical"

#: The checks that are settled by code, tests and generated artifacts rather
#: than by a measurement. One table, so nothing can drift between the check that
#: is built and the class it is rendered under: :func:`collect_claim_evidence`
#: reads this set, and a test asserts every check key appears in exactly one
#: class.
IMPLEMENTATION_CHECKS = frozenset(
    {
        "experiment_definition",
        "multimodal_ranker_exists",
        "audio_and_transcript_modalities",
        "candidate_generation_pipeline",
        "multimodal_feature_preprocessing",
        "labelling_infrastructure",
        "label_freeze_implemented",
        "frozen_held_out_test_set",
        "experiment_matrix",
        "validation_checkpoint_selection",
        "classical_baseline_implemented",
        "ndcg_crosscheck_implemented",
        "bootstrap_uncertainty_implemented",
...
def code_root(repo_root: Path) -> Path:
    """The checkout whose committed code the implementation claims describe.

    An implementation claim is a property of this repository, not of whatever
    artifact tree a caller points at. When ``repo_root`` is a checkout it is
    used directly; when it is not -- a temporary artifacts directory, say -- the
    claims are still answered against the checkout the running package was
    installed from, and fall back to ``repo_root`` (so they report as
    unestablished) when there is no such checkout.
    """
    candidate = Path(repo_root)
    if (candidate / "ml" / "src" / "slotify_rank").is_dir():
        return candidate
    installed = _PACKAGE_ROOT.parents[2]
    return installed if (installed / "ml" / "configs").is_dir() else candidate
...
def package_tests_root(repo_root: Path) -> Path:
    """The committed test package. Used to prove a capability is exercised."""
    return code_root(repo_root) / "ml" / "tests"


def referenced_symbols(path: Path, symbols: Sequence[str]) -> list[str]:
    """Which of ``symbols`` the module at ``path`` actually references.

    Parsed, not grepped, for the same reason as :func:`importing_modules`: a name
    that only appears in a docstring is not a use. Matches an imported name, an
    attribute access (``metrics.ndcg_score``) or a bare name in code.
    """
    file_path = Path(path)
    wanted = set(symbols)
    if not file_path.is_file() or not wanted:
        return []
    try:
...
def tests_covering(tests_root: Path, module_suffix: str) -> list[str]:
    """Committed test modules that import ``slotify_rank.<module_suffix>``.

    A capability that no test imports is code nobody runs, so an implementation
    claim that cannot name a test here is reported as unsupported rather than
    supported by the file's mere existence.
    """
    root = Path(tests_root)
    target = f"slotify_rank.{module_suffix}"
    found: list[str] = []
    if not root.is_dir():
        return found
...
@dataclass
class ClaimCheck:
    """One claim, its verdict, and where the verdict came from.

    ``evidence_class`` separates the two questions a reviewer asks. An
    :data:`IMPLEMENTATION` check answers "does this exist and run?"; an
    :data:`EMPIRICAL` check answers "what did it measure?". They are rendered in
    separate tables so a PASS on the first can never be read as the second.
    """
...
class ClaimEvidence:
    ...
    def check(self, key: str) -> ClaimCheck:
            ...
            "summary": {
                ...
                "implementation": {
                    status: sum(
                        ...
                        if c.evidence_class == IMPLEMENTATION and c.status == status
                    )
                    for status in (PASS, FAIL, NOT_MEASURED)
                },
                "empirical": {
                    status: sum(
                        1
                        for c in self.checks
                        if c.evidence_class == EMPIRICAL and c.status == status
                    )
                    for status in (PASS, FAIL, NOT_MEASURED)
                },
            },
...
def collect_claim_evidence(
    ...
    """Read every artifact and decide each headline claim's verdict."""
    root = Path(repo_root)
    ...
    # threshold typed into the checker.
    config_path = (
        Path(experiment_config_path)
        if experiment_config_path
        else root / "ml" / "configs" / "experiment_v2.yaml"
    )
    ...
    def add(
        ...
        checks.append(
            ClaimCheck(
                ...
                detail=detail,
                evidence_class=(
                    IMPLEMENTATION if key in IMPLEMENTATION_CHECKS else EMPIRICAL
                ),
    ...
    # -- 2b. candidate generation ------------------------------------------
    code = code_root(root)
    source_root_for_impl = package_source_root(code)
    tests_root = package_tests_root(code)
    generated_total = measurements["generated_candidate_count"]
    by_source = (candidate_statistics or {}).get("candidates_by_primary_source") or {}
    generation_tests = tests_covering(tests_root, "candidates")
    generation_ok = (
        isinstance(generated_total, int)
        and generated_total > 0
        and bool(by_source)
        and bool(generation_tests)
    )
    add(
        "candidate_generation_pipeline",
        "Deterministic candidate generation runs over the corpus and records "
        "which generator proposed each breakpoint.",
        PASS if generation_ok else NOT_MEASURED,
        (
            f"{generated_total} candidate(s) generated across "
            f"{len(by_source)} generator(s) ({', '.join(sorted(by_source))}), "
            f"covered by {len(generation_tests)} test module(s)."
            if generation_ok
            else "No candidate statistics artifact records generated candidates "
            "by generator."
        ),
        {
            "generated_candidate_count": generated_total,
            "candidates_by_primary_source": by_source,
            "tests": generation_tests,
        },
        [read.get("candidate_statistics", "")],
    )

    # -- 2c. multimodal preprocessing --------------------------------------
    feature_block = feature_statistics.get("features") or {}
    handcrafted_count = feature_block.get("handcrafted_feature_count")
    feature_tests = sorted(
        set(tests_covering(tests_root, "features") + tests_covering(tests_root, "embeddings"))
    )
    preprocessing_ok = bool(
        handcrafted_count
        and isinstance(measurements["complete_multimodal_candidate_count"], int)
        and measurements["complete_multimodal_candidate_count"] > 0
        and feature_tests
    )
    add(
        "multimodal_feature_preprocessing",
        "Every candidate is preprocessed into one feature record holding "
        "handcrafted scalars, a speech representation and a transcript embedding.",
        PASS if preprocessing_ok else NOT_MEASURED,
        (
            f"{measurements['complete_multimodal_candidate_count']} candidate(s) "
            f"hold a complete record of {handcrafted_count} handcrafted scalars "
            f"plus both embeddings; covered by {len(feature_tests)} test module(s)."
            if preprocessing_ok
            else "No feature statistics artifact records complete multimodal records."
        ),
        {
            "handcrafted_feature_count": handcrafted_count or NOT_MEASURED,
            "complete_multimodal_candidate_count": measurements[
                "complete_multimodal_candidate_count"
            ],
            "audio_embedding_model": audio_model,
        ...
        [read.get("feature_statistics", ""), read.get("embedding_statistics", "")],
    )

    # -- 2d. labelling infrastructure --------------------------------------
    queue_target = queue_summary_artifact.get("target_unique")
    queue_unique = queue_summary_artifact.get("unique_candidate_count")
    queue_repeats = queue_summary_artifact.get("blind_repeat_presentations")
    labelling_tests = tests_covering(tests_root, "labelling")
    queue_built = bool(
        queue_unique
        and queue_target
        and int(queue_unique) >= int(queue_target)
        and queue_repeats
        and labelling_tests
    )
    add(
        "labelling_infrastructure",
        "A stratified, blind labelling round is built and servable: the queue "
        "reaches its target size and carries unmarked repeat presentations.",
        PASS if queue_built else (FAIL if queue_summary_artifact else NOT_MEASURED),
        (
            f"Queue {queue_summary_artifact.get('queue_version')} holds "
            f"{queue_unique} unique candidate(s) against a {queue_target} target, "
            f"served as {queue_summary_artifact.get('presentation_count')} "
            f"presentation(s) including {queue_repeats} blind repeat(s); covered by "
            f"{len(labelling_tests)} test module(s). This is the size of the queue, "
            "not a count of labels collected against it."
            if queue_built
            else (
                "A queue summary exists but does not reach its target size, carries "
                "no blind repeats, or no test covers the labelling package."
                if queue_summary_artifact
                else "No labelling queue summary artifact exists."
            )
        ),
        {
            "queue_version": queue_summary_artifact.get("queue_version", NOT_MEASURED),
            "target_unique": queue_target or NOT_MEASURED,
            "unique_candidate_count": queue_unique or NOT_MEASURED,
            "presentation_count": queue_summary_artifact.get(
                "presentation_count", NOT_MEASURED
            ),
            "blind_repeat_presentations": queue_repeats or NOT_MEASURED,
            "by_split": (queue_summary_artifact.get("coverage") or {}).get(
                "by_split", NOT_MEASURED
            ),
            "candidate_manifest_hash": queue_summary_artifact.get(
                "candidate_manifest_hash", NOT_MEASURED
            ),
            "split_manifest_hash": queue_summary_artifact.get(
                "split_manifest_hash", NOT_MEASURED
            ),
            "human_labels_collected_against_it": measurements[
                "human_labelled_candidate_count"
            ],
            "tests": labelling_tests,
        },
        [read.get("labelling_queue_summary", "")],
    )

    # -- 2e. immutable label freezing --------------------------------------
    freeze_path = source_root_for_impl / "experiment" / "freeze.py"
    freeze_tests = tests_covering(tests_root, "experiment")
    freeze_ok = freeze_path.is_file() and bool(freeze_tests)
    add(
        "label_freeze_implemented",
        "Labels are frozen into an immutable, hashed snapshot that a rerun cannot "
        "silently rewrite.",
        PASS if freeze_ok else NOT_MEASURED,
        (
            f"{_display(freeze_path, code)} writes a hashed snapshot; the "
            "experiment declares snapshot version "
            f"{(experiment_config.get('labels') or {}).get('snapshot_version')!r}; "
            f"covered by {len(freeze_tests)} test module(s)."
            if freeze_ok
            else "No label-freeze module, or no test covering it."
        ),
        {
            "module": (
                _display(freeze_path, code) if freeze_path.is_file() else NOT_MEASURED
            ),
            "declared_snapshot_version": (
                (experiment_config.get("labels") or {}).get("snapshot_version")
                or NOT_MEASURED
            ),
            "referenced_symbols": referenced_symbols(
                freeze_path, ["sha256_text", "FileExistsError", "atomic_write_bytes"]
            ),
            "tests": freeze_tests,
        },
        [read.get("experiment_config", "")],
    ...
    # -- 5b. the ablation matrix -------------------------------------------
    model_block = experiment_config.get("model") or {}
    ablations = list(model_block.get("ablations") or [])
    seeds = list(model_block.get("seeds") or [])
    config_dir = code / str(model_block.get("config_dir") or "ml/configs/models")
    present_variants = (
        sorted(path.stem for path in config_dir.glob("*.yaml"))
        if config_dir.is_dir()
        else []
    )
    variant_registry = source_root_for_impl / "models" / "variants.py"
    matrix_module = source_root_for_impl / "experiment" / "matrix.py"
    missing_variant_configs = [
        variant
        for variant in ablations
        if not any(stem.startswith(variant) for stem in present_variants)
    ]
    matrix_tests = tests_covering(tests_root, "experiment.matrix") or [
        name for name in tests_covering(tests_root, "experiment") if "matrix" in name
    ]
    matrix_ok = bool(
        ablations
        and seeds
        and not missing_variant_configs
        and matrix_module.is_file()
        and variant_registry.is_file()
        and matrix_tests
    )
    add(
        "experiment_matrix",
        "The experiment declares a full ablation matrix, every variant it names "
        "has a committed config and a registered implementation, and the matrix "
        "runner is tested.",
        PASS if matrix_ok else (FAIL if ablations else NOT_MEASURED),
        (
            f"{len(ablations)} variant(s) x {len(seeds)} seed(s) = "
            f"{len(ablations) * len(seeds)} cells; every variant has a config under "
            f"{model_block.get('config_dir')}; the runner is "
            f"{_display(matrix_module, code)}, covered by {len(matrix_tests)} test "
            "module(s). Declaring the matrix is not running it."
            if matrix_ok
            else (
                f"Variants without a committed config: {missing_variant_configs}."
                if missing_variant_configs
                else "The experiment declares no ablations, or the matrix runner "
                "or its tests are absent."
            )
        ),
        {
            "ablations": ablations,
            "seeds": seeds,
            "cell_count": len(ablations) * len(seeds),
            "headline_variant": model_block.get("headline_variant", NOT_MEASURED),
            "committed_model_configs": present_variants,
            "matrix_module": (
                _display(matrix_module, code) if matrix_module.is_file() else NOT_MEASURED
            ),
            "tests": matrix_tests,
        },
        [read.get("experiment_config", "")],
    ...
    # -- 5c. validation-only model selection --------------------------------
    checkpoint_metric = model_block.get("checkpoint_selection_metric")
    seed_selection = model_block.get("seed_selection")
    checkpoint_module = source_root_for_impl / "training" / "checkpoint.py"
    recorded_metric = run_summary.get("checkpoint_selection_metric") or run_summary.get(
        "best_metric"
    )
    selection_tests = tests_covering(tests_root, "training")
    selection_ok = bool(
        checkpoint_metric
        and seed_selection
        and str(checkpoint_metric).startswith("validation")
        and checkpoint_module.is_file()
        and selection_tests
    )
    add(
        "validation_checkpoint_selection",
        "Checkpoints and seeds are selected on validation only, by a rule fixed "
        "before the test split is read.",
        PASS if selection_ok else (FAIL if checkpoint_metric else NOT_MEASURED),
        (
            f"Checkpoints are selected on {checkpoint_metric} and the reported "
            f"seed is the {seed_selection}; both are declared in "
            f"{experiment_config.get('experiment_version')} and implemented in "
            f"{_display(checkpoint_module, code)}, covered by "
            f"{len(selection_tests)} test module(s)."
            if selection_ok
            else (
                f"The declared selection metric {checkpoint_metric!r} is not a "
                "validation metric, or the checkpoint module or its tests are absent."
                if checkpoint_metric
                else "The experiment declares no checkpoint selection metric."
            )
        ),
        {
            "checkpoint_selection_metric": checkpoint_metric or NOT_MEASURED,
            "seed_selection": seed_selection or NOT_MEASURED,
            "module": (
                _display(checkpoint_module, code)
                if checkpoint_module.is_file()
                else NOT_MEASURED
            ),
            "latest_run_best_epoch": run_summary.get("best_epoch", NOT_MEASURED),
            "latest_run_selection_metric": recorded_metric or NOT_MEASURED,
            "tests": selection_tests,
        },
        [
            read.get("experiment_config", ""),
            read.get("human_trained_run") or read.get("latest_training_run", ""),
    ...
    # -- 5d. the classical baseline, as code --------------------------------
    classical_module = source_root_for_impl / "baselines" / "classical.py"
    classical_symbols = referenced_symbols(
        classical_module,
        ["HistGradientBoostingRegressor", "GroupKFold", "ndcg_score"],
    )
    classical_tests = tests_covering(tests_root, "baselines")
    classical_impl_ok = bool(
        classical_module.is_file()
        and "HistGradientBoostingRegressor" in classical_symbols
        and "GroupKFold" in classical_symbols
        and classical_tests
    )
    add(
        "classical_baseline_implemented",
        "A scikit-learn classical baseline is implemented over the handcrafted "
        "features, tuned by group-aware cross-validation inside the training split.",
        PASS if classical_impl_ok else (FAIL if classical_module.is_file() else NOT_MEASURED),
        (
            f"{_display(classical_module, code)} fits a "
            "HistGradientBoostingRegressor selected by GroupKFold, covered by "
            f"{len(classical_tests)} test module(s). Whether it has been fitted on "
            "the canonical corpus is a separate, empirical question."
            if classical_impl_ok
            else (
                f"{_display(classical_module, code)} does not reference both "
                "HistGradientBoostingRegressor and GroupKFold, or no test covers it."
                if classical_module.is_file()
                else "No classical baseline module exists."
            )
        ),
        {
            "module": (
                _display(classical_module, code)
                if classical_module.is_file()
                else NOT_MEASURED
            ),
            "referenced_symbols": classical_symbols,
            "declared_classical_baseline": (
                (experiment_config.get("baselines") or {}).get("classical")
                or NOT_MEASURED
            ),
    ...
    # -- 5e. the independent metric cross-check, as code --------------------
    crosscheck_module = source_root_for_impl / "evaluation" / "crosscheck.py"
    crosscheck_symbols = referenced_symbols(crosscheck_module, ["ndcg_score"])
    crosscheck_tests = tests_covering(tests_root, "evaluation.crosscheck") or [
        name for name in tests_covering(tests_root, "evaluation") if "sklearn" in name
    ]
    crosscheck_impl_ok = bool(
        crosscheck_module.is_file() and crosscheck_symbols and crosscheck_tests
    )
    add(
        "ndcg_crosscheck_implemented",
        "The headline metric is cross-checked against sklearn.metrics.ndcg_score "
        "by an independent implementation, and publication is blocked if they "
        "disagree.",
        PASS if crosscheck_impl_ok else (FAIL if crosscheck_module.is_file() else NOT_MEASURED),
        (
            f"{_display(crosscheck_module, code)} calls sklearn.metrics.ndcg_score "
            f"against this project's own NDCG, covered by {len(crosscheck_tests)} "
            "test module(s). The experiment requires the cross-check before a "
            f"number is published: require_independent_metric_crosscheck="
            f"{(experiment_config.get('evaluation') or {}).get('require_independent_metric_crosscheck')}."
            if crosscheck_impl_ok
            else (
                f"{_display(crosscheck_module, code)} does not reference "
                "ndcg_score, or no test covers it."
                if crosscheck_module.is_file()
                else "No metric cross-check module exists."
            )
        ),
        {
            "module": (
                _display(crosscheck_module, code)
                if crosscheck_module.is_file()
                else NOT_MEASURED
            ),
            "referenced_symbols": crosscheck_symbols,
            "required_by_experiment": (experiment_config.get("evaluation") or {}).get(
                "require_independent_metric_crosscheck", NOT_MEASURED
            ),
            "tests": crosscheck_tests,
        },
        [read.get("experiment_config", "")],
    )

    # -- 5f. bootstrap uncertainty, as code ---------------------------------
    evaluation_block = experiment_config.get("evaluation") or {}
    compare_module = source_root_for_impl / "evaluation" / "compare.py"
    bootstrap_symbols = referenced_symbols(
        compare_module, ["bootstrap_relative_improvement", "percentile", "Random"]
    )
    bootstrap_tests = [
        name
        for name in tests_covering(tests_root, "evaluation")
        if "uncertain" in name or "compare" in name
    ]
    bootstrap_impl_ok = bool(
        compare_module.is_file()
        and evaluation_block.get("bootstrap_resamples")
        and evaluation_block.get("bootstrap_confidence")
        and bootstrap_tests
    )
    add(
        "bootstrap_uncertainty_implemented",
        "Held-out uncertainty is reported as a percentile bootstrap resampled "
        "over episodes, with the resample count, confidence and seed fixed in "
        "advance.",
        PASS if bootstrap_impl_ok else (FAIL if compare_module.is_file() else NOT_MEASURED),
        (
            f"{_display(compare_module, code)} resamples episodes; the experiment "
            f"fixes {evaluation_block.get('bootstrap_resamples')} resamples at "
            f"{evaluation_block.get('bootstrap_confidence')} confidence with seed "
            f"{evaluation_block.get('bootstrap_seed')}, covered by "
            f"{len(bootstrap_tests)} test module(s)."
            if bootstrap_impl_ok
            else "No comparison module, no declared bootstrap configuration, or no "
            "test covering it."
        ),
        {
            "module": (
                _display(compare_module, code) if compare_module.is_file() else NOT_MEASURED
            ),
            "bootstrap_resamples": evaluation_block.get(
                "bootstrap_resamples", NOT_MEASURED
            ),
            "bootstrap_confidence": evaluation_block.get(
                "bootstrap_confidence", NOT_MEASURED
            ),
            "bootstrap_seed": evaluation_block.get("bootstrap_seed", NOT_MEASURED),
            "referenced_symbols": bootstrap_symbols,
            "tests": bootstrap_tests,
        },
        [read.get("experiment_config", "")],
    )

    # -- 5g. the reproducibility manifest -----------------------------------
    hashes = experiment_manifest.get("artifact_hashes") or {}
    pinned = sorted(name for name, value in hashes.items() if value)
    manifest_ok = bool(experiment_manifest.get("config_digest") and pinned)
    add(
        "reproducibility_manifest",
        "The experiment resolves to a manifest that pins the exact bytes it will "
        "be reported against: a config digest plus a hash per input artifact.",
        PASS if manifest_ok else (FAIL if experiment_manifest else NOT_MEASURED),
        (
            f"{experiment_manifest.get('experiment_version')} resolves to config "
            f"digest {str(experiment_manifest.get('config_digest'))[:16]}... and "
            f"pins {len(pinned)} input artifact(s): {', '.join(pinned)}."
            if manifest_ok
            else (
                "A manifest exists but records no config digest or no artifact hashes."
                if experiment_manifest
                else "No experiment manifest has been resolved."
            )
        ),
        {
            "experiment_version": experiment_manifest.get(
                "experiment_version", NOT_MEASURED
            ),
            "config_digest": experiment_manifest.get("config_digest", NOT_MEASURED),
            "pinned_artifacts": pinned,
            "package_version": experiment_manifest.get("package_version", NOT_MEASURED),
        },
        [read.get("experiment_manifest", "")],
    )

    # -- 5h. the product surface --------------------------------------------
    frontend_pkg = _read_json(code / "frontend" / "package.json") or {}
    backend_pkg = _read_json(code / "backend" / "package.json") or {}
    frontend_scripts = frontend_pkg.get("scripts") or {}
    backend_scripts = backend_pkg.get("scripts") or {}
    frontend_deps = {**(frontend_pkg.get("dependencies") or {})}
    product_ok = bool(
        "react" in frontend_deps
        and "test" in frontend_scripts
        and "build" in frontend_scripts
        and "test" in backend_scripts
        and "typecheck" in backend_scripts
    )
    add(
        "product_surface",
        "The ranker is served through a TypeScript product: a React frontend and "
        "a Node API, each with a lint/typecheck/test command CI runs.",
        PASS if product_ok else (FAIL if frontend_pkg or backend_pkg else NOT_MEASURED),
        (
            f"frontend react {frontend_deps.get('react')} with scripts "
            f"{sorted(frontend_scripts)}; backend scripts {sorted(backend_scripts)}."
            if product_ok
            else "A workspace is missing, or lacks a test/typecheck/build script."
        ),
        {
            "frontend_react_version": frontend_deps.get("react", NOT_MEASURED),
            "frontend_scripts": sorted(frontend_scripts),
            "backend_scripts": sorted(backend_scripts),
        },
    ...
    if not isinstance(improvement, (int, float)) or minimum_improvement == NOT_MEASURED:
        ...
        threshold_detail = (
            ...
            f"{minimum_improvement} %. The headline claim is NOT supported. The "
    ...
    pyproject = package_pyproject(code)
    declared = declared_dependencies(pyproject)
    source_root = source_root_for_impl
    ...
    for library in LIBRARY_CLAIMS:
        ...
        add(
            library.key,
            (
                f"scikit-learn ran inside a published comparison: an "
                "independent NDCG cross-check and a fitted classical baseline."
                if library.key == "sklearn_used"
                else f"The AI/ML stack includes {library.label}."
            ),
            status,
            detail,
            {
...
def render_markdown(payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        "claim-evidence` (`npm run claim-evidence`), which reads the artifacts "
        ...
        f"  - implemented capabilities: {summary['implementation'][PASS]} PASS / "
        f"{summary['implementation'][FAIL]} FAIL / "
        f"{summary['implementation'][NOT_MEASURED]} {NOT_MEASURED}",
        f"  - empirical results: {summary['empirical'][PASS]} PASS / "
        f"{summary['empirical'][FAIL]} FAIL / "
        f"{summary['empirical'][NOT_MEASURED]} {NOT_MEASURED}",
        "",
        ...
        "",
        "Two kinds of claim are tracked and never mixed. An **implementation** "
        "claim is settled by committed code, the test that exercises it and the "
        "artifact it produces: it says a capability exists and runs, never that "
        "it measured anything. An **empirical** claim is one only a measurement "
        "can settle. A PASS in the first table is not evidence for anything in "
        "the second.",
        "",
    ...
    def _row(check: Mapping[str, Any]) -> str:
        detail = str(check["detail"]).replace("|", "\\|").replace("\n", " ")
        return f"| {check['claim']} | **{check['status']}** | {detail} |"

    for check in payload["checks"]:
        if check.get("evidence_class") == IMPLEMENTATION:
            lines.append(_row(check))
    ...
    for check in payload["checks"]:
        lines += [
            ...
            "",
            f"*{check.get('evidence_class', EMPIRICAL)} claim*",
            "",
            check["detail"],
            "",
            "```json",
...
def write_claim_evidence(
    directory: Path, evidence: ClaimEvidence
) -> dict[str, Path]:
    target = Path(directory)
    target.mkdir(parents=True, exist_ok=True)
    payload = evidence.to_dict()
    json_path = target / "claim_evidence.json"
    md_path = target / "claim_evidence.md"
    atomic_write_bytes(
        json_path,
    ...
    return {"claim_evidence.json": json_path, "claim_evidence.md": md_path}
