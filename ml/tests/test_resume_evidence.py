...
EXPERIMENT_CONFIG = REPO_ROOT / "ml" / "configs" / "experiment_resume_v1.yaml"
...
def _artifacts(
    ...
    *,
    ...
    _write(
        artifacts / "experiments" / "experiment-resume-v1.json",
        {
...
def _collect(root: Path, **kwargs):
    ...
    return collect_resume_evidence(
        repo_root=root,
...
def test_the_threshold_comes_from_the_committed_config_not_the_code(tmp_path: Path):
    ...
    strict = collect_resume_evidence(tmp_path, artifacts, EXPERIMENT_CONFIG)
    lenient = collect_resume_evidence(tmp_path, artifacts, relaxed)
...
def test_importing_modules_finds_real_imports_and_ignores_mentions_in_text():
    ...
    # resume_evidence.py names every library in prose but imports none of them.
    assert not any("resume_evidence" in name for name in sklearn_importers)
...
def test_the_markdown_and_the_json_cannot_disagree(tmp_path: Path):
    ...
    written = write_resume_evidence(tmp_path / "reports", evidence)
    payload = json.loads(
        written["resume_evidence.json"].read_text(encoding="utf-8")
    )
    markdown = written["resume_evidence.md"].read_text(encoding="utf-8")
...
def test_the_report_contains_no_hard_coded_resume_numbers():
