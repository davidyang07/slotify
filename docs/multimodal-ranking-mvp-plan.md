...
Intended final claims: a trained multimodal PyTorch ranker; improved NDCG@3 over
...
into a named artifact, tracked in §33 and in `docs/resume-claim-matrix.md`.
...
| Component | Why |
|---|---|
| `lib/merge-filter.ts`, `routes/merge.ts` | preview + export work; the resume claims never require touching them |
...
- MVP dev corpus (fast iteration): **8–12 h**. Portfolio corpus: **≥ 50 h**, reached by extending
  ...
  and resume matrix quote that number, never a target.
...
Three distinct quantities. **`Y` on the resume is defined as A1** and the definition is printed
verbatim next to the number in `final_results.md`, the model card, and the README.
...
**The measured number is reported as measured.** If it is 45 %, the resume says 45 %. The benchmark is
specified before any data is collected precisely so it cannot be tuned toward 80 %.
...
the audio gate on conversational" is exactly the kind of finding an interview asks for, and it comes
...
`artifacts/reports/final_results.md`. The README and resume matrix link to the artifact rather than
...
## 33. Resume claim-to-evidence matrix

Maintained canonically in `docs/resume-claim-matrix.md`. Summary:
...
Express feature-flag integration; dataset/model cards; claim matrix.
...
Deliverables: this document + `docs/resume-claim-matrix.md`.
...
row of the claim matrix links to a real artifact.
